   ...
   service cloud.firestore {
     match /databases/{database}/documents {
       ...
       match /bookings/{bookingId} {
         ...
         allow update: if isAuthenticated() && (resource.data.customerUid == request.auth.uid || isAdmin());
         allow delete: if isAdmin();
       }
       ...
       // Damage Claims - users can only access their own
       ...
       // Driver Ratings - authenticated read, admin write
...
3. n8n Webhook must have "Header Auth" enabled with x-api-key validation
4. All secrets should be moved to environment variables in production
5. Consider implementing CSP headers on your hosting platform
*/
...
function getRateLimitReset(key, windowMs = 60000) {
  if (!rateLimits[key]) return 0;
  const elapsed = Date.now() - rateLimits[key].firstRequest;
...
input.style.cssText = "position;opacity:0;top:50%;left:50%;width:1px;height:1px;";
input.click();
setTimeout(() => { input.style.cssText = "position;width:0;height:0;"; }, 500);
}
...
═══════════════════════════════════════════════ */
function signupUser() {
if (!checkRateLimit("signup_otp", 3, 300000)) {
  showError("signupError", "⚠️ Too many OTP requests. Please try again later.");
  return;
}
...
"x-api-key": "packzen-webhook-key-2026"
},
body: JSON.stringify({
...
// Define pickupLocation and dropLocation for API call
const pickupLocation = pickupField;
const dropLocation = dropField;
...
if (!checkRateLimit("review_" + (currentUser?.uid || "anon"), 2, 3600000)) {
  showError("reviewMsg", "⚠️ You can only submit 2 reviews per hour.");
  return;
}
...
const adminSnap = await window.firebase.db.collection("users").doc(currentUser.uid).get();
...
if (!currentUser) { openAuthModal("login"); return; }
openDashboard();
setTimeout(() => switchDashTab("profile", document.querySelector(".dash-tab(4)")), 300);
}
...
try {
await window._firebase.db.collection("bookings").doc(ratingBookingDocId).update({ driverRating, driverFeedback, ratedAt: firebase.firestore.FieldValue.serverTimestamp() });
...
if (token && currentUser) await window._firebase.db.collection("users").doc(currentUser.uid).update({ fcmToken, fcmUpdatedAt: firebase.firestore.FieldValue.serverTimestamp() });
} catch(e) {}
}
...
img.style.cssText = "width:70px;height:70px;object-fit;border-radius:8px;border:2px solid var(--border-light)";
